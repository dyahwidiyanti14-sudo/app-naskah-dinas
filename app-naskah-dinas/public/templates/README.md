...
(huruf kecil semua, pakai tanda hubung `-`, bukan spasi). Total ada **6 file**,
...
**Surat Undangan Daerah tidak punya file template sendiri** — sesuai keputusan,
tab/formulir "Undangan" tetap ada sendiri, tapi saat dibuat file-nya me-reuse
`surat-dinas.docx` / `surat-dinas-lampiran.docx` (pilih lampiran otomatis kalau
ada isian "Daftar Pejabat/Pegawai yang Diundang"). Jadi jangan upload file
`undangan.docx` — tidak akan pernah dipakai oleh sistem.
...
| Surat Undangan Daerah | — | *(reuse `surat-dinas.docx` / `surat-dinas-lampiran.docx`, lihat catatan di atas)* |
...
### Surat Undangan Daerah → reuse `surat-dinas.docx` / `surat-dinas-lampiran.docx`
```
Tidak ada tag khusus undangan. Formulir Undangan (hari/tanggal, waktu, tempat
acara, acara) digabung jadi satu teks lalu dikirim lewat tag {alineaIsi} yang
sama seperti Surat Dinas — jadi tag di template ini TIDAK PERLU diubah sama
sekali, tag yang dipakai persis sama seperti Surat Dinas Daerah di atas.

Kalau ada isian "Daftar Pejabat/Pegawai yang Diundang", otomatis pakai versi
surat-dinas-lampiran.docx dan masuk ke Loop {#daftarLampiranList} yang sama.
```
